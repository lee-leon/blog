...
module.exports = ({
  ...
  const options = _.defaults(pluginOptions, defaults); // This will only work for markdown syntax image tags


  const markdownImageNodes = select(markdownAST, `image`); // This will also allow the use of html image tags

  const rawHtmlNodes = select(markdownAST, `html`); // Takes a node and generates the needed images and then returns
  // the needed HTML replacement for the image

  const generateImagesAndUpdateNode =
  /*#__PURE__*/
  function () {
    var _ref = (0, _asyncToGenerator2.default)(function* (node, resolve) {
  ...
  markdownImageNodes.map(node => new Promise(
  /*#__PURE__*/
  function () {
    var _ref2 = (0, _asyncToGenerator2.default)(function* (resolve, reject) {
      ...
      if (isRelativeUrl(node.url) && fileType !== `gif` && fileType !== `svg`) {
        const rawHTML = yield generateImagesAndUpdateNode(node, resolve);
  ...
  rawHtmlNodes.map(node => new Promise(
  /*#__PURE__*/
  function () {
    var _ref3 = (0, _asyncToGenerator2.default)(function* (resolve, reject) {
      ...
      for (var _i = 0; _i < imageRefs.length; _i++) {
        ...
        if (isRelativeUrl(formattedImgTag.url) && fileType !== `gif` && fileType !== `svg`) {
          const rawHTML = yield generateImagesAndUpdateNode(formattedImgTag, resolve);
