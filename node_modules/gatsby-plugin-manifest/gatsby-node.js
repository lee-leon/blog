...
exports.onPostBuild = function (args, pluginOptions) {
  return new Promise(function (resolve, reject) {
    ...
    var manifest = (0, _extends2.default)({}, pluginOptions); // Delete options we won't pass to the manifest.webmanifest.

    delete manifest.plugins;
    delete manifest.icon; // If icons are not manually defined, use the default icon set.
    ...
    var iconPath = path.join("public", manifest.icons[0].src.substring(0, manifest.icons[0].src.lastIndexOf("/"))); //create destination directory if it doesn't exist

    if (!fs.existsSync(iconPath)) {
      fs.mkdirSync(iconPath);
    }

    fs.writeFileSync(path.join("public", "manifest.webmanifest"), (0, _stringify.default)(manifest)); // Only auto-generate icons if a src icon is defined.
