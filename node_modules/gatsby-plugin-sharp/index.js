...
const processFile = (file, jobs, cb, reporter) => {
  ...
  function () {
    var _ref = (0, _asyncToGenerator2.default)(function* (job) {
      ...
      if (args.duotone) {
        clonedPipeline = yield duotone(args.duotone, job.file.extension, clonedPipeline);
      }
      ...
      if (job.file.extension === `png` && args.toFormat === `` || args.toFormat === `png`) {
        ...
        clonedPipeline.toBuffer().then(sharpBuffer => imagemin.buffer(sharpBuffer, {
          ...
          fs.writeFile(job.outputPath, imageminBuffer, onFinish);
        }).catch(onFinish)).catch(onFinish); // any other format (jpeg, tiff) - don't compress it just handle output
      } else {
...
function _notMemoizedbase() {
  _notMemoizedbase = (0, _asyncToGenerator2.default)(function* ({
    ...
    if (options.duotone) {
      pipeline = yield duotone(options.duotone, file.extension, pipeline);
    }
...
function _fluid() {
  _fluid = (0, _asyncToGenerator2.default)(function* ({
    ...
    const presentationHeight = Math.round(presentationWidth * (height / width)); // If the users didn't set default sizes, we'll make one.
    ...
    const filteredSizes = fluidSizes.filter(size => size < width); // Add the original image to ensure the largest image possible
    // is available for small images. Also so we can link to
    // the original image.

    filteredSizes.push(width); // Sort sizes for prettiness.
    ...
    const images = sortedSizes.map(size => {
      const arrrgs = Object.assign({}, options, {
        width: Math.round(size) // Queue sizes for processing.
...
function _fixed() {
  _fixed = (0, _asyncToGenerator2.default)(function* ({
    ...
    const options = healOptions(args, {}); // Create sizes for different resolutions — we do 1x, 1.5x, 2x, and 3x.
    ...
    const dimensions = getImageSize(file);
    const filteredSizes = sizes.filter(size => size <= dimensions.width); // If there's no fluid images after filtering (e.g. image is smaller than what's
    ...
    const images = sortedSizes.map(size => {
      const arrrgs = Object.assign({}, options, {
        width: Math.round(size) // Queue images for processing.
